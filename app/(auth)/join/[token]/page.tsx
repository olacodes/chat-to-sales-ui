...
import { getTraderInvite, signupWithPhone, verifyOtp } from '@/lib/auth/service';
import type { TraderInvite } from '@/lib/auth/types';

// ─── Categories (same as /signup) ────────────────────────────────────────────
...
export default function JoinPage({ params }: Props) {
  ...
  const [invite, setInvite] = useState<TraderInvite | null>(null);

  useEffect(() => {
    let cancelled = false;
    getTraderInvite(token)
      .then((data) => {
        if (cancelled) return;
        setInvite(data);
      ...
      <Card className="w-full max-w-md">
        <CardBody className="p-8 text-center space-y-4">
          <div
            ...
            style={{ backgroundColor: 'var(--ds-danger-bg)' }}
          >
            <svg
              className="h-6 w-6"
          ...
          <p className="text-sm" style={{ color: 'var(--ds-text-secondary)' }}>
            Ask the person who shared it to send you a new invite link.
          </p>
          <Link
            href="/signup"
          ...
          <>
            <div className="mb-6 text-center">
              ...
              <p className="mt-1 text-sm" style={{ color: 'var(--ds-text-secondary)' }}>
                {invite?.referrer_name
                  ? `${invite.referrer_name} invited you to ChatToSales`
                  : 'You were invited to start selling on WhatsApp'}
              </p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleFormSubmit} noValidate>
              ...
              <div>
                ...
                <p className="mt-1 text-xs" style={{ color: 'var(--ds-text-tertiary)' }}>
                  {invite?.phone_number
                    ? 'Pre-filled from your invite. Edit if needed.'
